...
#[test]
fn filters_matching_records_from_stdin() {
    let output = Command::new(env!("CARGO_BIN_EXE_selr"))
        .args(["2", "b"])
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
...
#[test]
fn filters_matching_records_from_file() {
    let file_path = write_temp_file("selr-filters-file-records", b"a b c\nx y z\n");
...
fn resolves_nf_per_record() {
    ...
    assert!(output.status.success());
    assert_eq!(output.stdout, b"a done\nx y done\n");
    assert_eq!(output.stderr, b"");
}
...
#[test]
fn empty_string_matches_all_records() {
    let output = Command::new(env!("CARGO_BIN_EXE_selr"))
        .args(["1", ""])
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
...
fn rejects_zero_selector() {
    ...
    assert_eq!(
        output.stderr,
        b"Error(108)[selr] : selector 0 is not supported by this command\n"
    );
}
...
fn rejects_range_selector() {
    ...
    assert_eq!(
        output.stderr,
        b"Error(108)[selr] : range selectors are not supported by this command\n"
    );
}
...
fn rejects_missing_field() {
    ...
    assert_eq!(
        output.stderr,
        b"Error(112)[selr] : selector resolved to a non-existent field\n"
    );
}
...
fn rejects_unterminated_final_record() {
    ...
    assert_eq!(
        output.stderr,
        b"Error(104)[selr] : final record is not terminated by LF\n"
    );
}
...
fn rejects_invalid_argument_count() {
    ...
    assert_eq!(
        output.stderr,
        b"Error(133)[selr] : usage: selr <fldnum> <str> [<file>]\n"
    );
}
